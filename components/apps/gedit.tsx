...
export class Gedit extends Component<Record<string, never>, GeditState> {
    ...
    componentDidMount(): void {
        emailjs.init(process.env.NEXT_PUBLIC_USER_ID as string);
    }

    sendMessage = async (): Promise<void> => {
        let name = $('#sender-name').val() as string;
        let subject = $('#sender-subject').val() as string;
        let message = $('#sender-message').val() as string;
            ...
            <div className="w-full h-full relative flex flex-col bg-ub-cool-grey text-white select-none">
                ...
                <div className="relative flex-grow flex flex-col bg-ub-gedit-dark font-normal windowMainScreen">
                    ...
                    <div className="relative flex-grow">
                        <textarea id="sender-message" className=" w-full gedit-message font-light text-sm resize-none h-full windowMainScreen outline-none tracking-wider pl-6 py-1 bg-transparent" placeholder="Message" spellCheck={false} autoComplete="none" />
...
export const displayGedit = (): React.ReactNode => {
    return <Gedit />;
};
