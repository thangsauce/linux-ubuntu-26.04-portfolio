...
let renderApps = (props: SideBarProps) => {
    let sideBarAppsJsx: React.ReactNode[] = [];
...
export default function SideBar(props: SideBarProps) {
    ...
    return (
        <>
            <div className={(props.hide ? " -translate-x-full " : "") + " absolute transform duration-300 select-none z-40 left-0 top-0 h-full pt-7 w-auto flex flex-col justify-start items-center border-r border-black border-opacity-30 bg-ub-grey rounded-r-2xl backdrop-blur-sm"}>
                {
                    (
            ...
            <div onMouseEnter={showSideBar} onMouseLeave={hideSideBar} onTouchStart={showSideBar} onTouchEnd={hideSideBar} className={"w-6 h-full absolute top-0 left-0 bg-transparent z-50"}></div>
        </>
    )
}
    ...
    return (
        ...
        >
            <div className="relative">
                <img width="28px" height="28px" className="w-7" src="./themes/Yaru/system/view-app-ubuntu.svg" alt="Ubuntu view app" style={{filter: "brightness(0) invert(1)"}} />
                <div
                    className={
